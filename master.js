$( document ).ready(function() {
	...
	$('.plus_sign').click(function(){
		if($('input[name=schoolLevel]:radio:checked').val()=='highschool'&&$('input[id=inputEmail]:text').val().length>0){
		...
		}else if($('input[name=schoolLevel]:radio:checked').val()=='college'&&$('input[id=inputEmail]:text').val().length>0){
			...
			div_2 = '<div class="noschool alert alert-danger alert-dismissable col-lg-9 col-lg-offset-2" role="alert"> <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>Please enter your level of education and school name</div>';
		}
